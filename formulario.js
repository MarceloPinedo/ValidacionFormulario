...
const validarFormulario=(e)=>{
    //console.log(e.target.name);// aca me muestra el nombre del imput que puse en el formulario
...
inputs.forEach((input)=>{
    input.addEventListener('keyup',validarFormulario);//cuando presiono una tecla se ejecuta el evento
    input.addEventListener('blur',validarFormulario);//cuando hago click afuera del input
});

formulario.addEventListener('submit',(e)=>{
    e.preventDefault();// hace que no se ejecute en la lado del servidor todavia.
// aca se hace la validacion que todo lo ingresado sea correcto 
    const terminos=document.getElementById('terminos');
    if(campos.usuario && campos.nombre && campos.password && campos.correo && campos.telefono && terminos.checked){
        formulario.reset()// reinicio todos los campos del formulario
